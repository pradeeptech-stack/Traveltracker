...
public class UserService {
	...
	public User login() {
		...
		try (Connection conn = DataBaseUtil.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
			...
			if (rs.next()) {
				...
				if (failedCount >= 5 || "Locked" .equals(accountStatus)) {
	...
	private void incrementFailedAttempts(String email, int failedCount) {
		String sql;
		if(failedCount +1 >= 5) {
			sql = "UPDATE users SET failed_count = ?, account_status = 'Locked' where email = ?";
		} else {
			sql = "UPDATE users SET failed_count = ? where email = ?";
		}
